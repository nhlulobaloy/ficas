...
import { getAssignedIncidents, getAssignedToPreliminary, getInvestigators, insertIncident, verifyAccess, verifyInsert } from '../models/incidentModel.js';
...
export const getIncidents = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const rows = await getAssignedToPreliminary(limit, offset);
...
export const getIncidentsAssigned = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await getAssignedIncidents(tokenId)
...
export const verifyAccessCreate = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await verifyInsert(userIdFromToken);
...
export const verifyAccessAssign = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await verifyAccess(userIdFromToken);
