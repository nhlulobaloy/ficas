...
export const referToDepartment = async (req, res) => {
    try {
        ...
        const sql = "UPDATE forensic_investigation_report SET referred_department = ?, status = ? WHERE id = ?";
...
export const returnPreliminary = async (req, res) => {
    try {
        ...
        const getForensicSql = "SELECT id, preliminary_id FROM forensic_investigation_report WHERE id = ?";
        ...
        const insertCommentSql = "INSERT INTO forensic_comments (forensic_id, user_id, user_name, comment) VALUES(?, ?, ?, ?)";
...
export const getForensic = async (req, res) => {
    ...
    try {
        ...
        const [getPreliIdResult] = await pool.execute("SELECT preliminary_id FROM forensic_investigation_report WHERE id = ?", [id]);
        ...
        const [result] = await pool.execute("SELECT * FROM forensic_investigation_report WHERE preliminary_id = ?", [preliminaryId]);
        if (result.length === 0)
            return res.status(404).json({ message: "Forensic case not found" });
        const forensic = result[0];
        // 3️⃣ Fetch comments
        const [comments] = await pool.execute("SELECT * FROM forensic_comments WHERE forensic_id = ? ORDER BY created_at ASC", [preliminaryId]);
...
export const updateForensic = async (req, res) => {
    ...
    const { background, abbreviations, annexures, individuals_featured, mandate, purpose_and_objective, scope_of_investigation, restrictions_and_limitations, legislative_policy_framework, investigation, interviews, financial_exposure, findings, recommendations, referred_department, general, conducted_by, status, } = req.body;
    try {
    ...
    `;
        ...
        const [updatedRow] = await pool.execute("SELECT * FROM forensic_investigation_report WHERE preliminary_id = ?", [preliminaryId]);
...
select count(*) as total
    ...
    `;
        ...
        for (let item of result) {
            const [comments] = await pool.query(`SELECT id, forensic_id, user_id, user_name, comment, created_at
         FROM forensic_comments
         WHERE forensic_id = ?
         ORDER BY created_at ASC`, [item.id]);
...
export const getForensicReview = async (req, res) => {
    ...
    `;
        ...
        // Fetch all comments
        const [comments] = await pool.query(`SELECT * FROM forensic_comments WHERE forensic_id = ? ORDER BY created_at ASC`, [forensic.id]);
        forensic.comments = comments || [];
...
export const getForensics = async (req, res) => {
    ...
    try {
        ...
        for (let c of cases) {
            const [comments] = await connection.execute("SELECT * FROM forensic_investigation_report WHERE preliminary_id = ? ORDER BY created_at ASC", [c.preliminary_id]);
            c.comments = comments || [];
        }
        res.status(200).json({
            message: "Success",
