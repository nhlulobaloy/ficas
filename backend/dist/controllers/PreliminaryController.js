...
export const createPreli = async (req, res) => {
    try {
        ...
        const { incident_id, mandate, allegations, acknowledgement_details, case_category, case_subcategory, domicile_department, key_stakeholders, incident_scene, amount_involved, persons_to_notify, persons_featured, legal_framework, preliminary_findings, annexures, recommendations, conducted_by, reviewed_by, approved_by, status, } = data;
        // Insert into database
        const [result] = await pool.query(`INSERT INTO preliminary_investigations 
...
export const getPreli = async (req, res) => {
    ...
    `;
        ...
        const prelim = rows[0];
        // Fetch all comments
        const [comments] = await pool.query(`SELECT * FROM preliminary_comments WHERE preliminary_id = ? ORDER BY created_at ASC`, [prelim.id]);
...
export const updatePreli = async (req, res) => {
    ...
    try {
        ...
        const [incidentRows] = await pool.query(`SELECT assigned_to FROM incidents WHERE id = ?`, [incidentId]);
        ...
        const [preliRows] = await pool.query(`SELECT status FROM preliminary_investigations WHERE incident_id = ?`, [incidentId]);
        ...
        /* 3️⃣ UPDATE */
        await pool.query(`UPDATE preliminary_investigations
...
export const approvePreliminary = async (req, res) => {
    try {
        ...
        const connection = await pool.getConnection();
        const [existing] = await connection.execute('SELECT id FROM preliminary_investigations WHERE id = ?', [preli_id]);
        if (existing.length === 0) {
            connection.release();
            return res.status(404).json({ error: 'Preliminary investigation not found' });
        }
        const [result] = await connection.execute(`UPDATE preliminary_investigations 
       SET status = 'approved', 
           approved_by = ?,
           updated_at = NOW()
       WHERE id = ?`, [approved_by, preli_id]);
...
export const verifyAccessUpdatePreliminary = async (req, res) => {
    try {
...
export const closeCase = async (req, res) => {
    try {
        ...
        const sql = "UPDATE preliminary_investigations SET status = ? WHERE id = ?";
...
export const getIncidentsAssigned = async (req, res) => {
    try {
       ...
       LIMIT ? OFFSET ?
       `;
        const [result] = await pool.execute(sql, [tokenId, limit, offset]);
        const total = getTotal[0].total;
