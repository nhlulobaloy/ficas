...
export const getFraudCase = async (req, res) => {
    const { id } = req.params;
    try {
        const [getFraudId,] = await pool.execute('SELECT id FROM fraud_prevention WHERE forensic_id = ?', [id]);
        const FraudId = getFraudId[0].id;
        // 2. Fetch the main fraud prevention record
        const [fraudRows] = await pool.execute(`SELECT
          forensic_id,
          executive_summary,
          abbreviations,
          annexures,
       ...
       WHERE id = ?`, [FraudId]);
        ...
        const [commentRows,] = await pool.execute(`SELECT fc.id, fc.comment, fc.user_id, fc.user_name, fc.created_at
       FROM fraud_prevention_comments fc
       WHERE fc.fraud_id = ?
       ORDER BY fc.created_at ASC`, [FraudId]);
...
export const updateCase = async (req, res) => {
    ...
    try {
        ...
        const [getFraudId] = await pool.execute('SELECT id, status FROM fraud_prevention WHERE forensic_id = ?', [id]);
        ...
        const { executive_summary, abbreviations, annexures, individuals_featured, mandate, purpose_and_objective, scope_of_investigation, restrictions_and_limitations, legislative_policy_framework, review_methodology, tone_at_the_top, review_details, sequence_of_events, findings, conclusions, recommendations, general, fraud_prevention, status, // frontend can still send draft or review
        assigned_to, } = req.body;
        // Update the case
        await pool.query(`UPDATE fraud_prevention SET
        executive_summary = ?,
        abbreviations = ?,
        annexures = ?,
        individuals_featured = ?,
...
export const getFraudReview = async (req, res) => {
    try {
        ...
        const [cases] = await pool.query(`SELECT 
         fp.*,
         u.name AS assigned_investigator_name
       FROM fraud_prevention fp
       LEFT JOIN users u ON fp.assigned_to = u.id
...
export const verifyReview = async (req, res) => {
    try {
        ...
        if (verify[0].fraud_prevention_review == 0)
...
export const verifyDraft = async (req, res) => {
    try {
        ...
        if (verify[0].fraud_prevention_draft == 0)
            return res.sendStatus(403);
        if (verify.length <= 0)
            return res.sendStatus(401);
        return res.sendStatus(200);
