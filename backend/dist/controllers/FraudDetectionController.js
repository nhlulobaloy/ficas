...
export const getDetectionCase = async (req, res) => {
    const { id } = req.params;
    try {
        const [getDetectionId] = await pool.execute('SELECT id FROM fraud_detection WHERE fraud_prevention_id = ?', [id]);
        //if case not found return not found error 
        if (getDetectionId.length <= 0)
            return res.sendStatus(404);
        const DetectionId = getDetectionId[0].id;
        // 2. Fetch the main fraud detection record
        const [detectionRows] = await pool.execute(`SELECT 
          fraud_prevention_id,
          background,
          findings,
          conclusion,
       ...
       ORDER BY fc.created_at ASC`, [DetectionId]);
...
export const updateCase = async (req, res) => {
    ...
    try {
        ...
        const [getDetectionId] = await pool.execute('SELECT id, status FROM fraud_detection WHERE fraud_prevention_id = ?', [id]);
        ...
        const { background, findings, conclusion, abbreviations_terms_definitions, annexures, mandate, review_objective, approach_and_procedures_performed, restrictions_and_limitations, data_analyst_methodology, applicable_legislation_policies, observations, recommendations, general, status, // frontend can still send draft or review
        assigned_to, referred_department } = req.body;
        // Update the case
        await pool.query(`UPDATE fraud_detection SET
        background = ?,
        findings = ?,
        conclusion = ?,
        abbreviations_terms_definitions = ?,
