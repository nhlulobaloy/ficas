...
export const getUsers = async (req, res) => {
    const [results] = await pool.query('SELECT u.*, a.* FROM users u LEFT JOIN access_rights a ON u.id = a.user_id');
...
export const updateUser = async (req, res) => {
    ...
    try {
        ...
        const { id, name, role, user_id, create_incident, assign_incident, update_preliminary, review_preliminary, review_forensic, fraud_prevention_draft, fraud_prevention_review, fraud_detection_draft, fraud_detection_review, } = StringUser;
        // Validate user exists
        const [validateUser,] = await connection.query(`SELECT user_id FROM access_rights WHERE user_id = ?`, [user_id]);
