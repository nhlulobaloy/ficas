...
export const createIncident = async (req, res, next) => {
    try {
        const { name, contact, date, time, location, email, selectedCategory, details, suspectDetails, sapsNumber, } = req.body;
        // 1. Insert incident
        const sql = `
    ...
    `;
        ...
        await pool.query(`INSERT INTO preliminary_investigations (incident_id) VALUES (?)`, [result.insertId]);
        // 5. Response
...
export const getIncident = async (req, res, next) => {
    ...
    `;
        ...
        const [preliRows,] = await pool.query(`SELECT id, status FROM preliminary_investigations WHERE incident_id = ?`, [incident_id]);
        if (preliRows.length > 0) {
            const preliId = preliRows[0].id;
            // Fetch all comments linked to this preliminary investigation
            const [commentRows,] = await pool.query(`SELECT id, preliminary_id, user_id, user_name, comment, created_at
         FROM preliminary_comments
         WHERE preliminary_id = ?
         ORDER BY created_at ASC`, [preliId]);
...
export const updateIncident = async (req, res, next) => {
    try {
        ...
        if (referred_department !== undefined) {
            ...
            updateValues.push(referred_department === '' ? null : referred_department);
        }
