...
interface FraudDetectionInvestigation {
  id: number;
...
export default function FraudDetection() {
  const [data, setData] = useState<FraudDetectionInvestigation[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [sessionMessage, setSessionMessage] = useState("");
  const [selectedInvestigation, setSelectedInvestigation] = useState<FraudDetectionInvestigation | null>(null);
  ...
  useEffect(() => {
    const fetchInvestigations = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/fraud/detection?page=${currentPage}&limit=${limit}`, {
  ...
  const viewDetails = (item: FraudDetectionInvestigation) => {
  ...
  return (
    <div className="fraud-dashboard">
      <div className="dashboard-header">
        <div>
          <h2 className="dashboard-title">Fraud Detection Investigations</h2>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <div className="modal-buttons">
              ...
              <button
                onClick={() => {
                  navigate(`/draft/fraud/detection/${selectedInvestigation.id}`, { 
